...
const GameIntroPanel: React.FC<GameIntroPanelProps> = ({
  ...
  return (
    <motion.div
      layout
      className="relative flex h-auto w-full min-w-0 max-w-full flex-col justify-center text-center lg:ml-auto lg:h-[34rem] lg:w-full lg:text-right xl:w-[34rem]"
      transition={{ layout: languageLayoutTransition }}
    >
      <motion.div
        layout="position"
        className="flex items-center justify-center lg:justify-end mb-8 sm:mb-12"
        transition={{ layout: languageLayoutTransition }}
      >
        <motion.h1
      ...
      <motion.p
        layout="position"
        className="ml-auto max-w-full whitespace-normal break-words text-base sm:text-lg md:text-xl text-intimate font-light mb-8 sm:mb-12 leading-relaxed transition-colors duration-700 lg:max-w-[44ch]"
        style={{ color: uiColor }}
        transition={{ layout: languageLayoutTransition }}
      >
